...
ifneq ($(DEVICE_OVERLAYS),)
    DT_OFFICIAL := $(shell grep -h 'vie_official_build">true<' $(DEVICE_OVERLAYS) 2>/dev/null)
    ifneq ($(DT_OFFICIAL),)
        VIE_DEVICES_XML := vendor/vieOTA/devices.xml
        ifneq ($(wildcard $(VIE_DEVICES_XML)),)
            IS_OFFICIAL := $(shell grep '<device>$(LINEAGE_BUILD)</device>' $(VIE_DEVICES_XML) 2>/dev/null)
            ifneq ($(IS_OFFICIAL),)
